        ...
        if(moviesList.length>0){
            moviesList.forEach((movie)=>{
                axios.get(`https://api.themoviedb.org/3/movie/${movie.id}/reviews?api_key=${API_KEY}&language=en-US`).then((res)=>{
                    const reviewsWithMovieInfo = res.data.results.map((review) => ({
                        ...review,
                        moviePoster: movie.poster_path,
                        movieTitle: movie.title,
                    }));
                    setReviewsList((prevReview) => [...prevReview, ...reviewsWithMovieInfo]);
                })
            })
        }
    },[moviesList])
                        ...
                        <Card.Body className="review-body">
                            <div className="review_div">
                                <div className="heading_div">
                                    ...
                                    <h6 style={{margin:"0rem"}}>{review.author_details.name || review.author_details.username || "Anonymous"}</h6>
                                </div>
